...
type PolicyEditableKey = 'organization' | 'purpose' | 'scope' | 'definitions' | 'approved' | 'prohibited' | 'confidentiality' | 'humanReview' | 'verification' | 'disclosure' | 'accountability' | 'security' | 'vendors' | 'incidents' | 'recordkeeping' | 'training' | 'reviewSchedule';
...
const initialPolicyDraft: PolicyDraft = {
  organization: '', industry: '', policyOwner: '', effectiveDate: '', reviewDate: '', status: 'Draft', purpose: '', scope: '', definitions: '', approved: '', prohibited: '', confidentiality: '', humanReview: '', verification: '', disclosure: '', accountability: '', security: '', vendors: '', incidents: '', recordkeeping: '', training: '', reviewSchedule: '', readinessChecks: {}, version: 'v1', complete: false,
};
...
const policySections = [
  ['purpose', '1. Purpose'], ['scope', '2. Scope'], ['definitions', '3. Definitions'], ['approved', '4. Approved AI uses'], ['prohibited', '5. Prohibited AI uses'], ['confidentiality', '6. Data and confidentiality'], ['humanReview', '7. Human oversight'], ['verification', '8. Verification'], ['disclosure', '9. Transparency and disclosure'], ['accountability', '10. Accountability'], ['security', '11. Security'], ['vendors', '12. Vendor management'], ['incidents', '13. Incident reporting'], ['recordkeeping', '14. Recordkeeping'], ['training', '15. Training'], ['reviewSchedule', '16. Policy review'],
];
...
export default function Home() {
  ...
  const renderEthicsModule = () => <>
    ...
    <section className="lmsPanel activityPanel"><div className="panelBar"><h3>Can I Put This Into AI?</h3><span>Privacy and data activity</span></div><p>Classify each example based on organizational policy and authorization. These are educational prompts, not legal advice.</p><div className="privacyGrid">{privacyExamples.map(([item, , reason]) => <label key={item}>{item}<select value={privacyChoices[item] || ''} onChange={(event) => setPrivacyChoices((current) => ({ ...current, [item]: event.target.value }))}><option value="">Choose</option><option>SAFE TO USE</option><option>USE CAUTION / VERIFY POLICY</option><option>DO NOT ENTER</option></select><small>{privacyChoices[item] ? reason : 'Choose a classification to reveal the reasoning.'}</small></label>)}</div></section>
  ...
  const renderGovernanceModule = () => <>
    ...
    <section className="lmsPanel activityPanel"><div className="panelBar"><h3>Before You Deploy · MEASURE</h3><span>Checklist</span></div><div className="checklistGrid">{['Has the system been tested?', 'Has data quality been evaluated?', 'Has potential bias been examined?', 'Have relevant stakeholders been considered?', 'Has human oversight been established?', 'Are limitations documented?', 'Is there a monitoring plan?'].map((item) => <label key={item}><input type="checkbox" checked={Boolean(governanceAnswers[`measure-${item}`])} onChange={() => setGovernanceAnswers((current) => ({ ...current, [`measure-${item}`]: !current[`measure-${item}`] }))} />{item}</label>)}</div></section>
    ...
    <section className="lmsPanel readinessPanel"><div className="panelBar"><h3>Policy Readiness</h3><span>{Object.values(policyDraft.readinessChecks).filter(Boolean).length} of 10</span></div><p>Readiness is an educational quality check, not legal compliance.</p><div className="checklistGrid">{['Approved uses', 'Prohibited uses', 'Confidential information rules', 'Human review', 'Verification', 'Accountability', 'Incident reporting', 'Policy owner', 'Review date', 'Monitoring'].map((item) => <label key={item}><input type="checkbox" checked={Boolean(policyDraft.readinessChecks[item])} onChange={() => updatePolicyReadiness(item)} />{item}</label>)}</div></section>
