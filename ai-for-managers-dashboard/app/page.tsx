...
const weeklyPlan = [
  ...
  { week: 8, dates: 'Oct 12–18', title: 'Understand customers', learn: 'Separate observed customer evidence from AI-generated guesses; use journey maps, segments, and privacy-safe research.', build: 'Create a customer-insight board with source, date, segment, observed need, and confidence for each insight; map one customer journey.', test: 'Check every insight against its source, include a missing or underserved voice, and label synthetic examples clearly.', manage: 'Choose one insight worth validating with real customers before changing a product or service.', present: 'Show one supported insight, the journey friction it reveals, and one question still needing customer research.', output: 'Source-linked insight board + journey map', studentQuestion: 'Can AI help me understand customers without making them up?', studentWin: 'Organize customer evidence and explain which conclusions still need real-world validation.', career: 'Support marketing, service design, and customer-experience decisions.', workload: '7–9 hours' },
  { week: 9, dates: 'Oct 19–25', title: 'Support people & teams', learn: 'Use AI for administrative team support while protecting privacy, fairness, and human decision rights.', build: 'Design a team-support workflow for meeting preparation, follow-up, or coaching questions using fictional or de-identified information.', test: 'Check for biased wording, unsupported inferences about people, private details, and decisions that need a manager or HR reviewer.', manage: 'Name the human reviewer and the point where AI output stops and a person decides.', present: 'Walk through the workflow and defend its allowed uses, prohibited uses, and escalation path.', output: 'Team-support workflow + boundary memo', studentQuestion: 'Where is AI useful—and dangerous—in managing people?', studentWin: 'Use AI to support a team process while keeping employment decisions with accountable people.', career: 'Manage teams and HR-related processes more responsibly.', workload: '7–9 hours' },
...
const focusedPractice: Record<number, { scenario: string; task: string; evidence: string; boundary: string }> = {
  8: {
    scenario: 'A service team has anonymized support-ticket themes and a small set of consented interview notes.',
    task: 'Group recurring needs, then map what a customer tries to do, where friction appears, and what evidence supports each step.',
    evidence: 'For every insight, record the source and mark whether it is observed, inferred, or only a synthetic example.',
    boundary: 'Do not treat invented personas or AI-generated quotations as customer research. Validate the proposed change with real customers.'
  },
  9: {
    scenario: 'A manager needs a clearer meeting agenda and follow-up plan for a fictional project team.',
    task: 'Draft an agenda, action-item template, and neutral coaching questions. Assign a person to review each output before use.',
    evidence: 'Check that the wording is factual, inclusive, and based only on information the team is allowed to use.',
    boundary: 'Do not use AI output to rank, hire, fire, discipline, or rate a person. Escalate sensitive concerns to the responsible manager or HR process.'
  }
};
              ...
              <section className="moduleDetail">
                ...
                {activePractice && (
                  <section className="lmsPanel moduleOutcome" aria-label={`Week ${selectedWeek} practice`}>
                    ...
                    <p><strong>Evidence check:</strong> {activePractice.evidence}</p>
                    <p><strong>Decision boundary:</strong> {activePractice.boundary}</p>
                  </section>
                )}
              </section>
            </div>
