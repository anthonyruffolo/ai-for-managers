...
type ModuleItem = { id: string; title: string; description: string; type: 'lesson' | 'practice' | 'case' | 'build' | 'assessment' | 'quiz' | 'assignment' | 'dropbox' | 'submit' | 'reflect' | 'resource' | 'connect'; time?: string };
...
const standardModule = (week: number, overview: string, objectives: string[], artifact: string, buildDescription: string, lessons: string[], practice: string, assessment: string, submission: string) => ({
  ...
  items: [
    ...
    ...(week === 10 ? [
      ...
      { id: 'dropbox-10-lesson-1', title: 'Dropbox · Academic Integrity Decision', description: 'Submit your 100–150 word Academic Integrity Decision before continuing to Lesson 2.', type: 'dropbox' as const },
    ] : []),
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setQuizSecondsRemaining(600);
    const timer = window.setInterval(() => {
      setQuizSecondsRemaining((current) => Math.max(0, current - 1));
    }, 1000);
  ...
  function renderStructuredWeek() {
    ...
    const lesson1AssignmentReady = lesson1AssignmentWordCount >= 100 && lesson1AssignmentWordCount <= 150;
    const lesson1DropboxSubmitted = Boolean(structuredAnswers['week10-dropbox-lesson-1-submitted']);
    const canContinueFromSelectedItem = selectedItem?.id !== 'dropbox-10-lesson-1' || lesson1DropboxSubmitted;
    const typeLabel: Record<ModuleItem['type'], string> = { lesson: 'LEARN', practice: 'PRACTICE', case: 'APPLY', build: 'BUILD', assessment: 'ASSESS', quiz: `QUIZ · ${quizTimerLabel} remaining`, assignment: 'ASSIGNMENT', dropbox: 'DROPBOX', submit: 'SUBMIT', reflect: 'REFLECT', resource: 'RESOURCES', connect: 'CONNECT' };
    ...
    if (selectedItem) return <>
      ...
      {selectedWeek === 10 && selectedItem.type === 'dropbox' && selectedItem.id === 'dropbox-10-lesson-1' && (() => {
        const submitted = Boolean(structuredAnswers['week10-dropbox-lesson-1-submitted']);
        return <section className="lmsPanel structuredLesson"><div className="panelBar"><h3>Dropbox · Academic Integrity Decision</h3><span>{submitted ? 'Submitted' : 'Not submitted'}</span></div><p>Submit your Lesson 1 assignment response here before continuing to Lesson 2.</p><div className="lessonCallout"><strong>Submission check</strong><span>{lesson1AssignmentReady ? 'Your response is within the required 100–150 word range.' : 'Your response must be between 100 and 150 words before submission.'}</span></div><textarea value={lesson1AssignmentResponse} onChange={(event) => answerStructured('week10-lesson1-assignment-response', event.target.value)} disabled={submitted} placeholder="Paste or finish your Academic Integrity Decision response here." /><div className="formActions"><span>{lesson1AssignmentWordCount} words</span><button className="primaryAction" type="button" disabled={submitted || !lesson1AssignmentReady} onClick={() => { answerStructured('week10-dropbox-lesson-1-submitted', true); completeModuleItem('dropbox-10-lesson-1'); }}>{submitted ? 'Submitted' : 'Submit to Dropbox'}</button></div></section>;
      })()}
      ...
      {selectedItem.type !== 'lesson' && selectedItem.type !== 'build' && selectedItem.type !== 'practice' && selectedItem.type !== 'case' && selectedItem.type !== 'connect' && selectedItem.type !== 'assessment' && selectedItem.type !== 'quiz' && selectedItem.type !== 'assignment' && selectedItem.type !== 'dropbox' && <section className="lmsPanel structuredLesson"><div className="panelBar"><h3>{typeLabel[selectedItem.type]} workspace</h3><span>Work here, then mark complete</span></div><p>{selectedItem.description}</p><div className="moduleWorkPrompt"><strong>What to record</strong><span>Your decision, evidence, affected stakeholders, responsible owner, safeguards, and what would change your recommendation.</span><textarea placeholder="Record your reasoning or submission notes here." /></div></section>}
      <div className="lessonNav"><button type="button" onClick={() => setModuleItem(structuredModule.items[Math.max(0, structuredModule.items.findIndex((item) => item.id === selectedItem.id) - 1)].id)}>← Previous</button><button className="primaryAction" type="button" onClick={() => completeModuleItem(selectedItem.id)}>{moduleCompletions[`${selectedWeek}-${selectedItem.id}`] ? 'Complete' : 'Mark complete'}</button><button type="button" disabled={!canContinueFromSelectedItem} onClick={() => setModuleItem(structuredModule.items[Math.min(structuredModule.items.length - 1, structuredModule.items.findIndex((item) => item.id === selectedItem.id) + 1)].id)}>Next →</button></div>
    </>;

    const groups = ['lesson', 'practice', 'case', 'build', 'connect', 'quiz', 'assignment', 'dropbox', 'assessment', 'submit', 'reflect', 'resource'] as ModuleItem['type'][];
    return <>
