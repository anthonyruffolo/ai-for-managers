...
export default function Home() {
  ...
  function renderStructuredWeek() {
    ...
    if (selectedItem) return <>
      ...
      {selectedItem.type === 'assessment' && selectedWeek === 1 && <section className="lmsPanel structuredLesson"><div className="panelBar"><h3>Quick Knowledge Check</h3><span>Answer before building</span></div><p>Which statement is most accurate?</p><div className="choiceStack">{['An LLM retrieves facts from a perfect database.', 'A hallucination is an answer that sounds plausible but is unsupported or false.', 'Generative AI and machine learning are unrelated.', 'If an AI tool is confident, a manager can skip verification.'].map((choice) => <button className={structuredAnswers.week1Knowledge === choice ? 'selected' : ''} type="button" onClick={() => answerStructured('week1Knowledge', choice)} key={choice}>{choice}</button>)}</div></section>}
